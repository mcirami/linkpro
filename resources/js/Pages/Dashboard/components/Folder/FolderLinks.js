...
const FolderLinks = ({
    ...
    useLayoutEffect(() => {
        ...
        if (targetRef.current && userLinks[arrayIndex].links.length > 0) {
            setSize({
                height: getColHeight(),
                width: getColWidth(),
            })
        ...
        function handleResize() {
            ...
            setTimeout(() => {
                ...
                if (icons.length > 0 ) {
                    ...
                    if (userLinks[arrayIndex].links.length < 4) {
                        divHeight += 20;
                    }
        ...
        ({ pageX, pageY }) => {
            ...
            if (isPressed) {
                ...
                const row = clamp(
                    ...
                    0,
                    Math.floor(userLinks[arrayIndex].links.length / 3)
                );
                let index = row * 3 + col;

                const newOrder = reinsert(
                    userLinks[arrayIndex].links,
                    userLinks[arrayIndex].links.findIndex((link) => link.position === lastPress),
                    index,
                );
    ...
    useEffect(() => {
        if (initialRender.current) {
            ...
            const packets = {
                userLinks: userLinks[arrayIndex].links,
            }
        ...
        <>
            ...
            <div className="icons_wrap add_icons icons folder">
                ...
                {userLinks[arrayIndex].links ? userLinks[arrayIndex].links.map((link, key) => {
                    ...
                    const linkID = originalArray[arrayIndex].links[key].id;
                    let displayIcon;
                    displayIcon = checkSubStatus(originalArray[arrayIndex].links[key].icon);
                                    ...
                                    <div className="column_content">
                                        ...
                                        <div className="my_row">
                                            <div className="switch_wrap">
                                                <Switch
                                                    onChange={(e) => handleChange(originalArray[arrayIndex].links[key])}
                                                    height={20}
                                                    checked={Boolean(originalArray[arrayIndex].links[key].active_status)}
                                                    onColor="#424fcf"
                                                    uncheckedIcon={false}
                                                    checkedIcon={false}
                                                />
                                                <div className="hover_text switch"><p>{Boolean(originalArray[arrayIndex].links[key].active_status) ? "Disable" : "Enable"} Icon</p></div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
